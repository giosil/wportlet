...
{	
	...
	{	
		...
		ResultSet rs = null;
		try {
			conn = ResourcesMgr.getConnection(portletConfig);
			stm = conn.createStatement();
			rs = stm.executeQuery(sSQL);

	    	List<String> listColumnNames = getColumnNames(parameters, request);
	...
	protected
	List<String> readColumnNames(ResultSet rs)
	  throws Exception
	{
	...
	protected
	List readData(ResultSet rs, int iMaxRows, List<String> listColNames)
	  throws Exception
	{
		ResultSetMetaData rsmd = rs.getMetaData();
		...
		while(rs.next()) {
		    ...
		    for (int i = 0; i < iColumnCount; i++){
		        ...
		        if(iColType == java.sql.Types.INTEGER ||
		        ...
		        if(iColType == java.sql.Types.DATE ||
	...
	protected abstract
	List<String> getColumnNames(Parameters parameters, ActionRequest request);
